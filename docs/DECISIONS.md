...
| **6.2** | UX de la app: instrucciones de uso + i18n del slider | Markdown compacto de 2 párrafos entre input image y rotation slider, bilingüe ES/EN, con (1) instrucción de rotación cuando la radiografía vino mal posicionada y (2) mini-leyenda de qué contiene cada tab (Binary/Vertebrae/Cobb Angle/Explainability). Patrón copia `header_markdown` / `explain_markdown`. Cierra además 2 labels hardcoded del Ciclo 5.5 (slider de rotación + botón Reset) via `gr.update(label=...)` / `gr.update(value=...)` en el handler de cambio de idioma — `gr.update` funciona en Gradio 5.50.0 para sliders/botones aunque el Ciclo 5.7 lo había declarado imposible. Tests: 75+1 (era 73+1). |
